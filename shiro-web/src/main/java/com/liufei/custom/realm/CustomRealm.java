package com.liufei.custom.realm;
...
public class CustomRealm extends AuthorizingRealm {
...

    {
        ...
        //加盐后的密文，salt值为“LPF”
        dataBaseUser.put("xiaoming","e10adc3949ba59abbe56e057f20f883e");
...
//        dataBasePermission.add("user:update");
        dataBasePermission.add("user:delete");
...
//        ((SimpleAuthenticationInfo) authenticationInfo).setCredentialsSalt(ByteSource.Util.bytes("lpf"));
    ...
    private Set<String> selectRole(String userName){
        System.out.println("从数据库中获取授权数据。。"+CustomRealm.class);
        List<String> listRole =userDao.queryRolesByUserName(userName);
        if(listRole.size()==0){
            return null;
        }
        Set<String> setRole = new HashSet<String>(listRole);
        return setRole;
    }
